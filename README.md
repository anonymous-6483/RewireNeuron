# RewireNeuron
Code for NeurIPS 2023 submission "Rewiring Neurons in Non-Stationary Environments".
...
## Getting started
Simply run the file `run.py` with the desired config available in [configs](configs/). Here is an example of our method Rewire applied to the HalfCheetah/forgetting scenario:
```shell
python run.py -cn=rewire scenario=halfcheetah/forgetting
```
...
* [algorithms](algorithms/) contains different RL / CL algorithms (e.g. SAC, EWC, or PackNet).
* [agents](agents/) contains salina agents (both policy and critic).
